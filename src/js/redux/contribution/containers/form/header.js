...
function mapDispatchToProps(dispatch) {
  return {
    ...
    new: (action) => {
      dispatch(fetchPostsIfNeeded("contribution/new/", types.NEW_CONTRIBUTION_FORM, action))
    },
    setViewStatus: (viewStatus) => {
      dispatch(setViewStatus(viewStatus))
    },
    save: (action) => {
      dispatch(fetchPostsIfNeeded("contribution/save/", types.SAVE_CONTRIBUTION_FORM, action, action))
    },
    addTag: (action) => {
      dispatch(fetchPostsIfNeeded("tag/add/", types.ADD_CONTRIBUTION_TAG, action))
    },
    deleteTag: (action) => {
      dispatch(fetchPostsIfNeeded("tag/delete/", types.DELETE_CONTRIBUTION_TAG, action))
    },
    addSound: (action) => {
      dispatch(fetchPostsIfNeeded("sound/add/", types.ADD_CONTRIBUTION_FORM_SOUND, action, action))
    }
  }
}
