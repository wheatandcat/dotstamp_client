...
export default class Menu extends Component {
  ...
  /**
     ...
     */
  reflect() {
    this.props.reflect({userContributionId: this.props.userContributionId, overwrite: this.overwrite.checked})
  ...
  getInformation() {
    ...
    if (this.props.soundShow.MakeMovie) {
      downloadMp3 = (
        <Button bsStyle="success" bsSize="small" href={getTopUrl() + "static/files/sound/" + this.props.userContributionId + ".mp3"} target="_blank">
          ダウンロード
        </Button>
      )
      downloadMp4 = (
        <Button bsStyle="success" bsSize="small" href={getTopUrl() + "static/files/movie/" + this.props.userContributionId + ".mp4"} target="_blank">
          ダウンロード
        </Button>
      )
    }
          ...
          <Well className={NoSpace}>
            ...
            <FormGroup>
              ...
              <Checkbox inputRef={ref => {
                this.overwrite = ref
              }} inline>既存のデータも上書して反映する</Checkbox>
            </FormGroup>
          </Well>
        </div>
        <div>
